...
function generateQuestionHtml(j, quest) {
    ...
    $.each(quest.solutions, function(k, solution) {
        html += generateSolutionHtml(j, k, solution);
    });
    html += "</fieldset></div>";
    return html;
}

function generateSolutionHtml(j, k, solution) {
    solution = "Solution " + k + ": " + solution;
    return '<input id="question_' + j + '_' + k + '_input" type="radio" name="question_' + j +'_solution" value="' + solution + '" required><label for="question_' + j + '_' + k + '_input">' + solution + '</label><br>';
}
