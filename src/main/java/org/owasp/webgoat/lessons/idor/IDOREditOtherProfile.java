...
public class IDOREditOtherProfile extends AssignmentEndpoint {
  ...
  public AttackResult completed(
    ...
    if (userSubmittedProfile.getUserId() != null) {
      if (userSubmittedProfile.getUserId().equals(authUserId)) {
        return failed(this).feedback("idor.edit.profile.failure4").build();
      }
      return handleOtherUserProfile(userSubmittedProfile, currentUserProfile);
    }
  ...
  private AttackResult handleOtherUserProfile(
      UserProfile userSubmittedProfile, UserProfile currentUserProfile) {
